...
3. **Build** - Implement with AI assistance
...
| Type | Tests | LTP E2E | Code Review |
|------|-------|---------|-------------|
...
```bash
pip install sdd-kit
sdd-kit init claude  # or cursor, codex, meli
```
...
Yes, for:
- MCP servers (, code review tool, LTP, dependency security scanner)
- `fury` CLI commands
...
```bash
#  (required)
claude mcp add --scope project --transport stdio  -- mcp-remote-proxy https://mcp.melioffice.com/namespaces/backend/mcp --transport http

# code review tool (optional)
claude mcp add --scope project --transport stdio code review tool -- mcp-remote-proxy https://genai-code-review-mcp.melioffice.com/mcp --transport http

# dependency security scanner (required)
claude mcp add --scope project --transport stdio dependency security scanner -- mcp-remote-proxy https://codeguard-mcp.melioffice.com/mcp --transport http
```
...
Check:
1. Are you in the project root? (not inside `.sdd-kit/`)
2. Is the  app created? (`fury info`)
3. Are you logged in? (``)
4. Is VPN connected?
...
The functional spec is built through a **structured AI interview** (4-6 questions). In this phase:
...
|------|----------|---------|-----------|
| `governance.md` | `~/.sdd-kit/standards/` | Framework **principles** (philosophy) | No |
| `PROJECT.md` | `meli/` | Project **configuration** (values) | Yes |
...
1. Go to https://web.furycloud.io/
...
### How does the framework discover existing  services?

During `/sdd.spec technical`, when the architecture recommends  services
(KVS, BigQueue, Cache, etc.), the framework:
1. Runs `fury services <type> list` (CLI first,  for services without CLI list)
...
Requirements: fury CLI installed, logged in, VPN connected.
If fury CLI unavailable, falls back to manual service naming.

### Which  services should I use?

Use the **fury-services-architect** skill which classifies your needs and loads expert reference data for 6 service families:

| Service Family | Services |
|----------------|----------|
| Database | KVS, QKVS, Cache, MySQL, PostgreSQL, NoSQL, NewSQL, GraphDB, TSMetrics, Oracle, BigQuery, DS (12) |
| Communication | BigQueue, Streams, Workqueues, Director, Jobs, Schedule Engine, Verdi Flows, Mails, Template Processing (9) |
...
For code implementation snippets, the **fury-snippets-expert** provides ready-to-use examples.
...
### Should I commit `meli/` folder?

Yes! The `meli/` folder contains:
...
5. **[ AI](https://web.furycloud.io/fury-ai)**: Ask questions about the documentation interactively
