...
```bash
# Check what failed
bash .sdd-kit/tools/validation/validate-functional.sh sdd/wip/[feature]
...
3. Re-run validator
4. If still failing, use `/sdd.spec functional --include` for AI assistance
...
```bash
# Check what failed
bash .sdd-kit/tools/validation/validate-technical.sh sdd/wip/[feature]

# Also check spec alignment
bash .sdd-kit/tools/validation/validate-spec-alignment.sh sdd/wip/[feature]
```
...
| Issue | Fix |
|-------|-----|
| Missing code compliance | Add  Platform Compliance section |
...
2. Check file permissions: ls -la .sdd-kit/tools/
3. Make executable: chmod +x .sdd-kit/tools/*.sh
...
Fix:
1. Run  validator: bash .sdd-kit/tools/validation/validate-code.sh .
2. Review each error
3. Common fixes:
...
| Get AI help with issues | `/sdd.spec functional --include` |
...
bash .sdd-kit/tools/validation/validate-code.sh .
```
